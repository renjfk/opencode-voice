...
import {
  ...
  clearBubbles,
  interBubblePauseMs,
  CONSTANTS as CHUNKER_CONSTANTS,
} from "./tts-chunker.js";
import { speakWithDeepgram, DEEPGRAM_DEFAULTS } from "./tts-deepgram.js";
import { openTtsPanel } from "./tts-panel.js";

// Load the v2 WhatsApp-style panel at module init. If @opentui/solid or
// its tree-sitter highlight asset can't be loaded in the current opencode
// runtime, we degrade silently and fall back to the DialogSelect panel.
let renderTtsPanelV2 = null;
try {
  const mod = await import("./tts-panel-v2.js");
  renderTtsPanelV2 = mod.renderTtsPanelV2;
} catch (err) {
  // .scm asset or solid renderer unavailable — fall back.
  if (typeof console !== "undefined" && console.warn) {
    console.warn(`[opencode-voice] v2 panel unavailable: ${err.message}`);
  }
}
...
export function registerTTS(api, kv, complete, prompts, logger) {
  ...
  function speak(text) {
    ...
    if (engine === "deepgram") {
      const apiKeyEnv = options?.deepgramApiKeyEnv || DEEPGRAM_DEFAULTS.apiKeyEnv;
      const apiKey = process.env[apiKeyEnv] || null;
      const model = options?.deepgramModel || DEEPGRAM_DEFAULTS.model;
      const container = options?.deepgramContainer || DEEPGRAM_DEFAULTS.container;
      logger?.log?.(
        "TTS",
        `Speak requested engine=deepgram chars=${line.length} model=${model}`,
        "debug",
      );
      return speakWithDeepgram({ text: line, apiKey, model, container, logger }).catch((err) => {
        logger?.log?.("TTS", `Deepgram speak failed: ${err.message}`, "error");
        toast(`Deepgram TTS failed: ${err.message}`, "error");
      });
    }
  ...
  // ---- Voice bubbles (WhatsApp-style chunked playback) ----

  function chunkedModeEnabled() {
    return kv.get("tts.chunked", "on") !== "off";
  }
  ...
  /**
   * Speak `text` as a sequence of short voice bubbles. Each bubble is
   * recorded in kv with its estimated duration so the user can replay
   * individual bubbles via /tts-bubbles.
   */
  async function speakChunked(text, meta = {}) {
    if (!text) return;
    ...
    logger?.log?.(
      "TTS",
      `Bubble playback requested chunks=${pieces.length} sourceChars=${clean.length}`,
      "debug",
    );

    const sessionID = meta.sessionID || null;
    const sessionTitle = meta.sessionTitle || null;
    const source = meta.source || "manual";

    if (pieces.length === 1) {
      // Short responses still get treated as a single bubble so the history
      // is consistent — users see "one bubble" in the dialog, not a phantom
      // long one that never actually existed.
      await speak(pieces[0]);
      recordBubble(kv, {
        text: pieces[0],
        duration: estimateDuration(pieces[0]),
        sessionID,
        sessionTitle,
        source,
      });
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (isPunctuationOnly(piece)) continue;
      const idxLabel = `${i + 1}/${total}`;
      toast(`Voice bubble ${idxLabel}`);
      const duration = estimateDuration(piece);
      recordBubble(kv, {
        text: piece,
        duration,
        sessionID,
      ...
      logger?.log?.(
        "TTS",
        `Speaking bubble ${idxLabel} chars=${piece.length} ~${duration}s`,
        "debug",
      );
      await speak(piece);
      if (i < total - 1) {
        await waitMs(interBubblePauseMs());
      }
    }
  }

  /**
   * Replay a previously recorded bubble. The bubble's text is normalized the
   * same way as a fresh speak() call (no chunking — bubbles are already
   * bounded) so the user hears exactly what was originally synthesized.
   */
  async function replayBubble(bubble) {
    if (!bubble || !bubble.text) {
      toast("Bubble is empty", "warning");
      return;
    }
    toast(`Replaying bubble (~${bubble.duration || estimateDuration(bubble.text)}s)`);
    await speak(bubble.text);
  }

  function formatBubbleLabel(bubble) {
    const dur = bubble.duration || estimateDuration(bubble.text);
    const stamp = new Date(bubble.ts || Date.now()).toISOString().slice(11, 16);
    const preview = bubble.text.length > 60 ? `${bubble.text.slice(0, 57)}...` : bubble.text;
    return `${stamp}  ${dur}s  ${preview}`;
  }
  ...
  api.event.on("session.idle", async (event) => {
    ...
    if (chunkedModeEnabled()) {
      const sessionTitle = await getSessionTitle(client, sessionID);
      const parts = [];
      if (sessionTitle) parts.push(`Session: ${sessionTitle}.`);
      parts.push(llmResult.text);
      parts.push("Ready for your input.");
      await speakChunked(parts.join(" "), {
        sessionID,
        sessionTitle,
        source: "auto",
      });
  ...
  async function speakLastResponse() {
    ...
    if (chunkedModeEnabled()) {
      toast("Speaking last response (bubbles)");
      await speakChunked(llmResult.text, { source: "manual" });
    } else {
  ...
  function openTtsPanelAny() {
    if (renderTtsPanelV2) {
      try {
        const element = renderTtsPanelV2(api, kv, {
          speak: (done) => {
            speakLastResponse().finally(done);
          },
          speakBubble: (bubble, done) => {
            replayBubble(bubble).finally(done);
          },
          stop: () => {
          ...
          toggleEngine: (done) => {
            ...
            toast(`Engine: ${next === "deepgram" ? "Deepgram Aura 2" : "Piper (local)"}`);
            done();
          },
          toggleMode: (done) => {
            ...
            toast(next === "on" ? "Auto-speak on" : "Manual-only");
            done();
          },
          toggleChunked: (done) => {
            ...
            if (next === "off") stopSpeech();
            toast(next === "on" ? "Voice bubbles on" : "Voice bubbles off");
            done();
          },
          toggleVoice: (done) => {
            const cur = kv.get("tts.voice", DEFAULT_TTS_VOICE);
            const keys = Object.keys(TTS_VOICES);
            const idx = keys.indexOf(cur);
            const next = keys[(idx + 1) % keys.length] || DEFAULT_TTS_VOICE;
            kv.set("tts.voice", next);
            const entry = TTS_VOICES[next] || TTS_VOICES[DEFAULT_TTS_VOICE];
            toast(`Voice: ${entry.label}`);
            done();
          },
          clearBubbles: () => {
            const list = kv.get("tts.bubbles", []) || [];
            clearBubbles(kv);
            toast(`Cleared ${list.length} bubble${list.length === 1 ? "" : "s"}`);
          },
        ...
        logger?.log?.("TTS", `v2 panel render failed: ${err.message}`, "warn");
      }
    }
    // Fallback: high-level DialogSelect panel (no @opentui deps).
    openTtsPanel(api, kv, { TTS_VOICES, DEFAULT_TTS_VOICE });
  }
  ...
  return [
    ...
    {
      title: "TTS: select engine",
      value: "tts.engine",
      description: "Switch between local Piper and Deepgram Aura 2 cloud TTS",
      slash: { name: "tts-engine" },
      onSelect() {
        const current = kv.get("tts.engine", "piper");
        const options = [
          {
            title: "Piper (local, free, requires on-disk voice model)",
            value: "piper",
            onSelect() {
              kv.set("tts.engine", "piper");
              toast("TTS engine: Piper (local)");
              api.ui.dialog.clear();
            },
          },
          {
            title: "Deepgram Aura 2 (cloud, requires DEEPGRAM_API_KEY in env)",
            value: "deepgram",
            onSelect() {
              kv.set("tts.engine", "deepgram");
              toast("TTS engine: Deepgram Aura 2 (cloud)");
              api.ui.dialog.clear();
            },
          },
        ];
    ...
    {
      title: "TTS: toggle voice bubbles",
      value: "tts.chunked",
      description: "Toggle WhatsApp-style short voice bubbles on/off (default on)",
      slash: { name: "tts-chunked" },
      onSelect() {
        const current = kv.get("tts.chunked", "on");
        const next = current === "on" ? "off" : "on";
        kv.set("tts.chunked", next);
        if (next === "off") stopSpeech();
        toast(next === "on" ? "Voice bubbles on" : "Voice bubbles off");
    ...
    },
    {
      ...
      value: "tts.panel",
      description: "Open the TTS control panel with WhatsApp-style voice feed",
      keybind: "<leader>t",
      slash: { name: "tts-panel" },
      onSelect() {
        openTtsPanelAny();
      },
    },
    {
      title: "TTS: show voice bubbles",
      value: "tts.bubbles",
      description: "Browse and replay the most recent voice bubbles",
      slash: { name: "tts-bubbles" },
      onSelect() {
        const bubbles = getBubbles(kv, { limit: CHUNKER_CONSTANTS.MAX_BUBBLES });
        if (bubbles.length === 0) {
          toast("No voice bubbles recorded yet");
          return;
        }
        api.ui.dialog.replace(() =>
          api.ui.DialogSelect({
            title: `Voice bubbles (${bubbles.length})`,
            current: bubbles[0].id,
            options: [
              {
                title: `Clear all (${bubbles.length})`,
                value: "__clear__",
                onSelect() {
                  clearBubbles(kv);
                  toast("Voice bubbles cleared");
                  api.ui.dialog.clear();
                },
              },
              ...bubbles.map((bubble, idx) => ({
                title: formatBubbleLabel(bubble),
                value: bubble.id,
                onSelect() {
                  api.ui.dialog.clear();
                  replayBubble(bubble);
