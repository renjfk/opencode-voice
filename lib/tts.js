...
export function registerTTS(api, kv, complete, prompts) {
  ...
  function speak(text) {
    ...
    if (!fs.existsSync(PIPER_BIN) || !fs.existsSync(voiceModel)) {
  ...
  api.event.on("permission.asked", async (event) => {
    ...
    const parts = [];
    if (sessionTitle) parts.push(`Session: ${sessionTitle}.`);
    parts.push("Permission requested. Please check your screen.");
    await speak(parts.join(" "));
  });
  ...
  api.event.on("question.asked", async (event) => {
    ...
    const parts = [];
    if (sessionTitle) parts.push(`Session: ${sessionTitle}.`);
    parts.push("A question needs your answer. Please check your screen.");
    await speak(parts.join(" "));
  });
