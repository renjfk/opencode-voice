...
async function getActiveSessionTitle(client) {
  try {
...
async function waitForSoxExit(timeoutMs = 2000) {
  ...
  while ((soxProc || soxPid) && Date.now() - start < timeoutMs) {
    await new Promise((r) => setTimeout(r, 100));
  }
  if (soxProc || soxPid) forceKillSox();
}
